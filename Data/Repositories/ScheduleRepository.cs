...
namespace Data.Repositories
{
    public class ScheduleRepository : IScheduleRepository
    {
        ...
        {
             var schedules = await _dbContext.Schedules.Include(x => x.Teacher).Include(s => s.Course).ThenInclude(c => c.CourseUsers).ThenInclude(x => x.User).Where(s => s.CourseID == courseId && !s.IsDeleted).ToListAsync();
            return schedules;
        }
        // It retrieves all schedules that are not marked as deleted, including their associated course information.
        public async Task<IEnumerable<Schedule>> GetAllSchedulesAsync()
        {
            var schedules = await _dbContext.Schedules.Include(x => x.Teacher).Include(s => s.Course).ThenInclude(c => c.CourseUsers).ThenInclude(x => x.User).Where(s => !s.IsDeleted).ToListAsync();
            return schedules;
        }
        ...
        {
            var schedule = await _dbContext.Schedules.Include(x => x.Teacher).Include(s => s.Course).ThenInclude(c => c.CourseUsers).ThenInclude(x => x.User).FirstOrDefaultAsync(s => s.ScheduleID == id && !s.IsDeleted);
            return schedule;
        }
        ...
        {
            var schedules = await _dbContext.Schedules.Include(x => x.Teacher).Include(s => s.Course).ThenInclude(c => c.CourseUsers).ThenInclude(x => x.User)
